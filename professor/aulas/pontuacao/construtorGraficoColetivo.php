...
if (isset($_GET['turma'])) {
	...
	$query_n = "SELECT s.nome as nome, AVG(r.resposta) as media FROM tbl_respostas r, tbl_perguntas p , tbl_sequencia_perguntas s , tbl_aulas a WHERE r.id_pergunta = p.id AND p.id_sequencia = s.id AND s.id = a.id_sequencia AND a.id_turma = '$turma' GROUP BY s.nome";
    ...
    $graph->SetYTitle('Comparação das medias das porcentagens de acertos coletivos(%)'); // seta o einxo Y e seu nome
